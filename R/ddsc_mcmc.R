...
#' 
#' The main function \code{ddsc_mcmc} in this package can be used to deconvolve 
#' a density with a unimodal and symmetric shape. The method is coined the name 
#' Constrained Bayes and is essentially a nonparametric approach. The speed is 
#' bumped by utilizing functions written in Rcpp. 
#'
...
#' default values are chosen if left empty.  
#' @return A vector containing the expected posterior density of the latent true 
#' variable evaluated on a grid chosen automatically in the function.
#' @examples 
#' df.t <- 5
#' x <- rt(n, df = df.t)
#' sd_u <- 1.29
...
ddsc_mcmc <- function(w, sd_u, n.burnin=1000, n.MCMC=5000, hpar=list(m=20, K=8, 
